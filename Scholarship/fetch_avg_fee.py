...
def fetch_graduate_average_tuition_fees():
    """Main function to fetch graduate average tuition fees for all colleges."""
    print("="*80)
    print("GRADUATE AVERAGE TUITION FEE EXTRACTOR")
    print("="*80)
    ...
    # Filter out colleges without URLs
    ...
    print(f"Found {total_colleges} colleges with URLs to process.\n")
    
    # Determine output file paths
    output_dir = os.path.dirname(json_file_path) if json_file_path else "."
    json_filename = os.path.join(output_dir, "graduate_average_tuition_fees.json")
    excel_filename = os.path.join(output_dir, "graduate_average_tuition_fees.xlsx")
    
    # Load existing results to resume from where we left off
    all_tuition_fees = load_existing_results(json_filename)
    
    # Filter out colleges that have already been processed (have a result with url)
    colleges_to_process = {}
    for college_name, url in colleges_with_urls.items():
        # Skip if already processed (has url and either tuition_fee or error)
        ...
        colleges_to_process[college_name] = url
    
    remaining_count = len(colleges_to_process)
    already_processed = total_colleges - remaining_count
    ...
    if remaining_count == 0:
        ...
        for idx, (college_name, url) in enumerate(colleges_to_process.items(), 1):
            ...
            if save_results_incrementally(json_filename, all_tuition_fees):
    ...
    if save_results_incrementally(json_filename, all_tuition_fees):
        ...
        print(f"⚠️  Warning: Could not save final results")
...
if __name__ == "__main__":
    """Run the script to extract graduate average tuition fees."""
